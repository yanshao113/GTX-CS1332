...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        ExternalChainingMapEntry<K,V> newPut = new ExternalChainingMapEntry<>(key,value);
        newPut.setNext(table[idx]);
        table[idx]=newPut;
        size++;
        return null;
    }
    ...
    private void resizeBackingTable(int length) {
        ...
        for(int i = 0; i < table.length; i++){
            if(table[i] != null){
                int idx = Math.abs(table[i].getKey().hashCode()% length);
                if(newTable[idx] !=null){
                    newTable[idx].setNext(new ExternalChainingMapEntry<>(table[i].getKey(),table[i].getValue()));
                }else{
                    newTable[idx] = new ExternalChainingMapEntry<>(table[i].getKey(),table[i].getValue());
                }
            }
        }
        table = newTable;
